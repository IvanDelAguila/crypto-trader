...
//
// createLearning(store) permite tener una instancia de aprendizaje ligada a un pool
// de capital independiente (ej. el motor de prueba SMC), para que no mezcle su
// historial con el del bot principal.
...
function createLearning(store) {
  ...
  return { getStrategyMultiplier, getStrategyOverview, getGlobalRiskThrottle };
}

const defaultStore    = require("./db");
const defaultLearning = createLearning(defaultStore);

module.exports = defaultLearning;
module.exports.createLearning = createLearning;
